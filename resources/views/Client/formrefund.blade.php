                ...
                return new Promise(function(resolve) {
                    ...
                    .done(function(response){
                        if(response.status == 'success'){
                            ...
                            swal('Info', 'Data Refund Gagal Di Simpan', 'info');
                        }
            ...
            enctype: 'multipart/form-data',
                ...
                success : function (data) {
                  if(data.status == 'success'){
                    ...
                    swal('Info', 'Data Refund Gagal Dikirim', 'info');
                  }
                }
              });
          break;
...
$('#FormDebitur').find('#tglPelunasan').on('change', function (e) {
    ...
    var date1 = new Date(tglakad);
    var date2 = new Date(tgllunas);
    var diffYears = date2.getFullYear()-date1.getFullYear();
    var diffMonths = date2.getMonth()-date1.getMonth();
    var diffDays = date2.getDate()-date1.getDate();

    var months = (diffYears*12 + diffMonths);
    var sisa = masakredit-months;
    // if(diffDays>0) {
    //     months += '.'+diffDays;
    // } else if(diffDays<0) {
    //     months--;
    ...
    var refundd = ((((40/100) * nilaipremi)/masakredit)*sisa).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');;
    var refund = (((nilai/100) * nilaipremi)/selisihtotal*selisih1).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
    // console.log(nilai, months, sisa, refundd);
    $('#FormDebitur').find('#nilaiRefund').val(refundd);
    // refund =  
